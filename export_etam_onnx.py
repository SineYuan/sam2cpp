...
from ETAM_Module import ImageEncoder, ImageDecoder, MemAttention, MemEncoder
...
def export_image_encoder(model,onnx_path):
    ...
    output_names = ["pix_feat","vision_feats","vision_pos_embed"]
    torch.onnx.export(
        model,
        input_img,
...
def export_image_decoder(model,onnx_path):
    ...
    input_name = ["point_coords","point_labels","image_height", "image_width", "image_embed"]
    ...
    torch.onnx.export(
        ...
        (point_coords,point_labels,image_height,image_width,image_embed),
...
onnx_output_path = "checkpoints/{}/".format("etam_s")
model_checkpoints_file = "checkpoints/efficienttam_s.pt"
model_config_file = "configs/efficienttam/efficienttam_s.yaml"
...
if __name__ == "__main__":
    ...
    model = build_efficienttam(args.config, args.checkpoint, device="cpu")

    image_encoder = ImageEncoder(model).cpu()
    export_image_encoder(image_encoder,args.outdir)

    image_decoder = ImageDecoder(model).cpu()
    export_image_decoder(image_decoder,args.outdir)

    mem_encoder   = MemEncoder(model).cpu()
    export_memory_encoder(mem_encoder,args.outdir)

    mem_attention = MemAttention(model).cpu()
